...
if __name__ == "__main__":
    ...
    # 执行命令：python test_case1.py --task 3 --machine 4 --position 6 --heuristic 0 --base_num 7
    ...
    tasks[9].release = [[3]]
    tasks[10].occupy = [[1], [4]]  #
    tasks[10].release = [[1], [2]]
    tasks[11].occupy = [[5]]  #
    tasks[11].release = [[5]]
    tasks[12].occupy = [[1], [6]]  #
    tasks[12].release = [[1], [4]]
    tasks[13].occupy = []  #
    tasks[13].release = [[6]]
    ...
    for t in range(args.task * args.base_num):  # 复制的任务的前驱和后继的id对应加上
        if t >= base_num:
            ...
            for occ_id, occ in enumerate(task_base.occupy_dependency):
                if occ != -1:
                    tasks[t].occupy_dependency[occ_id] = task_base.occupy_dependency[occ_id] + int(
                        t // base_num * base_num)
            for rel_id, rel in enumerate(task_base.release_dependency):
                if rel != -1:
                    tasks[t].release_dependency[rel_id] = task_base.release_dependency[rel_id] + int(
                        t // base_num * base_num)
    # 根据L1列表调度算法计算优先级
    ...
    while len(q) > 0:
        ...
        for occ_id, occ in enumerate(task.occupy):
            ...
            # 若当前task 选择了依赖约束：和其他task在相同的machine上执行   处理某些任务必须在相同设备上执行的约束。
                ...
                for pos in occ:
                    if positions[pos].status == 0 and positions[pos].machine in tasks[
                        task.occupy_dependency[occ_id]].machine:
                        positions[pos].status = 1
                        tmp_position.append(positions[pos].id)
                        tmp_machine.append(positions[pos].machine)
                        break
            ...
            if flag == 1 and flag2 == 1 and t.status == 0:
                ...
                if heuristics == 1:  # 启发式规则计算第二优先级
                    ...
                    # 若当前task 选择了依赖约束：和其他task在相同的machine上执行   处理某些任务必须在相同设备上执行的约束。
                        ...
                        for pos in task.occupy:
                            if positions[pos - 1].status == 0 and positions[pos].machine == tasks[
                                task.dependency].machine:
                    ...
                    for task in tasks:
                        ...
                        if task.dependency == -1:
                            ...
                            for pos in task.occupy:
                                if positions[pos - 1].status == 0 and positions[pos - 1].machine == tasks[
                                    task.dependency].machine:
                                    flag2 = 1
                        if len(task.occupy) == 0:
                            flag2 = 1
