...
if __name__ == "__main__":
    ...
    positions = [Position(p+1) for p in range(args.position)]
    ...
    for t in range(args.task*args.base_num):  # 复制的任务的前驱和后继的id对应加上
        if t<base_num: 
            ...
            task_base = tasks[t%base_num]
            if task_base.dependency != -1:
                tasks[t].dependency = task_base.dependency + int(t//base_num*base_num) 
            if task_base.dependency2 != -1:
                tasks[t].dependency2 = task_base.dependency2 + int(t//base_num*base_num) 
    ...
    for task in task_id:
        ...
        Tm[tasks[task].position-1] = begin_time  # Tm表示设备什么时候SS开始可用
