...
# Simulate According To Priority.  维护优先级队列，每次从任务中取优先级最高的任务进行调度，若出现队列为空情况则进行回溯。
    # 执行命令：python test_case5.py --task 10 --machine 4 --position 6 --heuristic 0 --base_num 9
    ...
    tasks[6].occupy = [[3]]  #
    tasks[6].release = [[3]]
    tasks[7].occupy = [[1],[0]]  # 0：冰箱1出Plate
    tasks[7].release = [[1],[2]]
    tasks[8].occupy = []
    tasks[8].release = [[0]]
    ...
    while len(q) > 0:
        ...
        if len(q) == 0 and step != len(tasks):
            ...
            q_id = copy.deepcopy(SAVED_PRIOR_QUEUE[step])
            q = [tasks[t_id] for t_id in q_id]
            print("RETURN　LAST　STEP !!","任务ID:",task.id, "当前step:",step)
            DEAD = 2
            decision = []
